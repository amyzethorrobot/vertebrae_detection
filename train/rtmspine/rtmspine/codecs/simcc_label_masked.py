...
class SimCCLabelMasked(BaseKeypointCodec):
    ...
    def encode(self,
        ...
        if self.mask_part > 0 and self.mask_part <= 1:
            keypoints_mask = np.ones(keypoints.shape[:2], dtype=np.float32)
            keypoints_num = keypoints.shape[1]
            top_idx = int(keypoints_num*self.mask_part)

            for mask_line in keypoints_mask:
                mask_line[:top_idx] = 0
                np.random.shuffle(mask_line)

            keypoints_visible = keypoints_visible * keypoints_mask
